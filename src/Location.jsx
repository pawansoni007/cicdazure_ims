...
const Location = () => {
  ...
  const columns = [
    {
      name: <strong>Location Name</strong>,
      selector: (row) => {
        if (row.id === editingId) {
          return (
            <input
              ...
              onChange={(e) => {
                ...
                setDetails((prevState) =>
                  prevState.map((item) =>
                    item.id === editingId ? updatedLocation : item
                  )
                );
              }}
            />
  ...
  function saveData() {
    ...
    if (editingId) {
      const updatedLocation = details.find((item) => item.id === editingId);
      axios
        .put(`${BASE_URL}/api/Location/Put/${editingId}`, {
      ...
      axios
        .post(`${BASE_URL}/api/Location/Post`, location, headers, {
        ...
        .then((res) => 
        {
          ...
          Object.keys(tempLocation).forEach((e) => (e === 'id' ? tempLocation[e] = 0 : tempLocation[e] = ""));
          setLocation(tempLocation);
          setMessage(`Location "${res?.data?.locationName}" added `);
          setShowPopup(true);
          mutate();
        })
        .catch((error) => {
